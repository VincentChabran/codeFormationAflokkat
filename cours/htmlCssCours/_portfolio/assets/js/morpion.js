...
for (let i = 0; i < grille.length; i++) {
    grille[i].case.addEventListener(
        "click",
        () => {
            ...
            if (!gagner) {
                ...
                if (grille[i].vide) {
                    ...
                    // tour du joueur2
                        ...
                        grille[i].case.classList.add("caseJoueur2");
                        grille[i].case.textContent = "O";
                        grille[i].vide = false;
                        nombreCasesPleine++;
                        verificationWin("caseJoueur2");
                        numeroJoueur--;
                    }
                } else {
...
function verificationWin(classe) {
    ...
    let conditionEntrer =
        (rawCases[0].classList.contains(classe) &&
            rawCases[1].classList.contains(classe) &&
            rawCases[2].classList.contains(classe)) ||
        (rawCases[3].classList.contains(classe) &&
            rawCases[4].classList.contains(classe) &&
            rawCases[5].classList.contains(classe)) ||
        (rawCases[6].classList.contains(classe) &&
            rawCases[7].classList.contains(classe) &&
            rawCases[8].classList.contains(classe)) ||
        (rawCases[0].classList.contains(classe) &&
            rawCases[3].classList.contains(classe) &&
            rawCases[6].classList.contains(classe)) ||
        (rawCases[1].classList.contains(classe) &&
...
function resetGrille() {
    for (let i = 0; i < grille.length; i++) {
        grille[i].case.classList.remove("caseJoueur1", "caseJoueur2");
    ...
    afficheTour.textContent = numeroJoueur == 1 ? `tour : ${nomJoueur1}` : `tour : ${nomJoueur2}`;
}
