"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DatabaseFilesResolver = void 0;
const common_1 = require("@nestjs/common");
const graphql_1 = require("@nestjs/graphql");
const database_files_service_1 = require("./database-files.service");
const graphql_upload_1 = require("graphql-upload");
const fs_1 = require("fs");
const databaseFile_entity_1 = require("./entities/databaseFile.entity");
const notifications_service_1 = require("../notifications/notifications.service");
const logs_service_1 = require("../logs/logs.service");
const graphql_subscriptions_1 = require("graphql-subscriptions");
let DatabaseFilesResolver = class DatabaseFilesResolver {
    constructor(databaseFilesService, notificationsService, logsService, pubSub) {
        this.databaseFilesService = databaseFilesService;
        this.notificationsService = notificationsService;
        this.logsService = logsService;
        this.pubSub = pubSub;
    }
    async uploadFile(reclamationId, file) {
        const { createReadStream, filename } = file;
        if (!(0, fs_1.existsSync)(`${process.cwd()}/../files/${reclamationId}`)) {
            (0, fs_1.mkdirSync)(`${process.cwd()}/../files/${reclamationId}`, {
                recursive: true,
            });
        }
        return new Promise(async (resolve, reject) => createReadStream()
            .pipe((0, fs_1.createWriteStream)(`${process.cwd()}/../files/${reclamationId}/${filename}`))
            .on("finish", async (res) => {
            console.log(res);
            const newFile = await this.databaseFilesService.create({
                filename,
                reclamationId,
            });
            await this.logsService.create({
                reclamationId,
                log: `Ajout du fichier ${filename}`,
            });
            const participants = await (await newFile.reclamation).participants;
            const participantsIds = participants.map(({ id }) => {
                return id;
            });
            this.pubSub.publish("refetching", {
                refetching: { ids: participantsIds },
            });
            for (let i = 0; i < participants.length; i++) {
                await this.notificationsService.create({
                    notification: `Une pièce jointe a été déposée pour la réclamation #${reclamationId}`,
                    reclamationId: reclamationId,
                    utilisateurId: participants[i].id,
                });
                this.pubSub.publish("sendNotification", {
                    sendNotification: { id: participants[i].id },
                });
            }
            resolve(true);
        })
            .on("error", (err) => {
            console.log(err);
            reject(false);
        }));
    }
    findAll() {
        return this.databaseFilesService.findAll();
    }
};
__decorate([
    (0, graphql_1.Mutation)(() => Boolean),
    __param(0, (0, graphql_1.Args)("reclamationId")),
    __param(1, (0, graphql_1.Args)({ name: "file", type: () => graphql_upload_1.GraphQLUpload })),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", Promise)
], DatabaseFilesResolver.prototype, "uploadFile", null);
__decorate([
    (0, graphql_1.Query)(() => [databaseFile_entity_1.default], { name: "databaseFiles" }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], DatabaseFilesResolver.prototype, "findAll", null);
DatabaseFilesResolver = __decorate([
    (0, graphql_1.Resolver)(),
    __param(3, (0, common_1.Inject)("PUB_SUB")),
    __metadata("design:paramtypes", [database_files_service_1.DatabaseFilesService,
        notifications_service_1.NotificationsService,
        logs_service_1.LogsService,
        graphql_subscriptions_1.PubSubEngine])
], DatabaseFilesResolver);
exports.DatabaseFilesResolver = DatabaseFilesResolver;
//# sourceMappingURL=database-files.resolver.js.map