const tabOptionSecteurActiviter = [
   'Non renseigné',
...
const generateOptionsSecteurActiviter = (): { value: string; label: string }[] => {
   const tab: { value: string; label: string }[] = [];
   tabOptionSecteurActiviter.forEach((el, i) => tab.push({ value: i < 9 ? `0${i + 1}` : `${i + 1}`, label: el }));

   return tab;
};

export const optionsSecteurActiviter = generateOptionsSecteurActiviter();
